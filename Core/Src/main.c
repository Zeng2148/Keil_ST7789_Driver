...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, BLK_Pin|DC_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : BLK_Pin DC_Pin */
  GPIO_InitStruct.Pin = BLK_Pin|DC_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void StartDisplayTask(void const * argument)
{
  /* USER CODE BEGIN StartDisplayTask */
	// 硬件初始化
	HAL_GPIO_WritePin(BLK_PORT, BLK_PIN, GPIO_PIN_SET); // 开启背光
  ...
  /* Infinite loop */
	while(1) 
	{
		...
		ST7789_DrawLine(10,20,200,200,COLOR_RED);
		osDelay(1000);
		ST7789_DrawImageTest();	
		osDelay(1000);
		ST7789_DrawChineseTest();
		osDelay(1000);

		ST7789_RotationDisplayTest();		
		osDelay(1000);		
	}
